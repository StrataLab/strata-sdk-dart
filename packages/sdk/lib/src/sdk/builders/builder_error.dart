...
class BuilderError implements Exception {
  ...
  factory BuilderError.userInputErrors(List<BuilderError> causes) =>
      BuilderError(causes.map((e) => e.message).join(", "),
          type: BuilderErrorType.userInputError);

  /// A Builder error indicating a runtime error.
  factory BuilderError.builderRuntimeError(String message,
          {Exception? cause}) =>
      BuilderError(message,
          type: BuilderErrorType.builderRuntimeError, exception: cause);
...
class BuilderRuntimeError extends BuilderError {
  BuilderRuntimeError(String super.message, Exception cause)
      : super(exception: cause);
}
